...
from vllm.model_executor.models.commandr import LayerNorm
...
class ModelForCasualLM(nn.Layer, ABC):
    ...
    @abstractmethod
    def set_state_dict(self, state_dict: Dict[str, Union[np.ndarray, torch.Tensor]]):
        """
        Load model parameters from a given state dictionary.

        Args:
            state_dict (dict[str, np.ndarray | torch.Tensor]):
                A dictionary containing model parameters, where keys are parameter names
                and values are NumPy arrays or torchtorch tensors.
        """
...
class Ernie45TAttention(nn.Layer):
    ...
    def __init__(self, llm_config: VllmConfig, layer_id: int, prefix: str) -> None:
        super().__init__()
...
class Ernie45TModel(nn.Layer):
    ...
    def load_state_dict(self, state_dict):
        ...
        for i in range(self.num_layers):
            print(f"Start load layer {i}")
            self.hidden_layers[i].load_state_dict(state_dict)

    def forward(
        self,
        ids_remove_padding: torch.Tensor,
        forward_meta: ForwardMeta,
    ):
        """ """
...
class ErnieForCausalLM(ModelForCasualLM):
    ...
    def __init__(self, llm_config: VllmConfig):
        """
        Args:
            llm_config (VllmConfig): Configurations for the LLM model.
        """
    ...
    def compute_logits(self, hidden_states: torch.Tensor):
        ...
        logits = torch.cast(logits, torch.float32)
